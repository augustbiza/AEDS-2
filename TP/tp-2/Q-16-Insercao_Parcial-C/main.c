...
int compare(Show* a, Show* b) {
    ...
    char titleA[300];
    char titleB[300];
    toLowerTitle(a->title, titleA);
    toLowerTitle(b->title, titleB);

    return strcmp(titleA, titleB);
}

void insertionPartial(Show* shows[], int n, int k, int* comp, int* mov) {
    ...
    for(int i = 1; i < n; i++) {
        ...
        Show* aux = shows[i]; (*mov)++;

        while(j >= 0 && compare(shows[j], aux) > 0) {
            (*comp)++;
            shows[j+1] = shows[j];
            j--;
        }(*comp)++;
...
int main() {
    ...
    int k = 10;
    insertionPartial(base, baseCount, k, &comp, &mov);

    clock_t fim = clock();

    double tempo = (double)(fim-inicio)/CLOCKS_PER_SEC;

    FILE *Log = fopen("./853033_insercaoParcial.txt","w");
