...
public class Main {
    ...
    static long mov = 0, comp = 0;

    public static void heap(ArrayList<Show> shows) {

        int n = shows.size();

        try {
            FileWriter Log = new FileWriter("853033_heapsort.txt");
            ...
            for (int i = n - 1; i > 0; i--) {
                Show aux = shows.get(0);
                shows.set(0, shows.get(i));
                shows.set(i, aux);
                mov += 3;
    ...
    public static void heapfy(ArrayList<Show> shows, int n, int i) {
        int maior = i;
        int esq = 2 * i + 1;
        int dir = 2 * i + 2;

        if (esq < n && compararShows(shows.get(esq), shows.get(maior)) > 0) {
            maior = esq;
        }

        if (dir < n && compararShows(shows.get(dir), shows.get(maior)) > 0) {
            maior = dir;
        }

        if (maior != i) {
            Show aux = shows.get(i);
            shows.set(i, shows.get(maior));
            shows.set(maior, aux);
            mov += 3;

            heapfy(shows, n, maior);
        }
    }


    public static int compararShows(Show a, Show b) {
        boolean aTemDirector = a.getDirector() != null && !a.getDirector().isBlank();
        boolean bTemDirector = b.getDirector() != null && !b.getDirector().isBlank();

        // 1. Se apenas um deles tem diretor, esse vem primeiro
        if (aTemDirector && !bTemDirector) return -1;
        if (!aTemDirector && bTemDirector) return 1;

        comp++;

        // 2. Se os dois têm diretor, comparar pelo primeiro nome do diretor
        if (aTemDirector && bTemDirector) {
            String aPrimeiroNome = a.getDirector().trim().split("\\s+")[0];
            String bPrimeiroNome = b.getDirector().trim().split("\\s+")[0];
            int cmp = aPrimeiroNome.compareToIgnoreCase(bPrimeiroNome);
        ...
        // 3. Se nenhum tem diretor, comparar pelo título
        return a.getTitle().compareToIgnoreCase(b.getTitle());
    }
