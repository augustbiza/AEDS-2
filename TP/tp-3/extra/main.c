...
int main(void) {
    ...
    Fila* disney = (Fila*)malloc(sizeof(Fila)); 
    criarFila(disney);
    for(int i = 0; i < baseCount; i++) inserir(disney, base[i]);
        ...
        if(strncmp(linha, "I ", 2) == 0) {
            ...
            for (int j = 0; j < showCount; j++) {
                if(strcmp(getShowId(shows[j]), idOp) == 0) {
                    inserir(disney, shows[j]);
                    break;
                }
            }
        } 
        else if(strcmp(linha, "R") == 0) {
            Show* removido = remover(disney);
            if(removido != NULL) {
                printf("(R) %s\n", removido->title);
            }
        }
